...
namespace ArduinoAutomator
{
    ...
    public class ArduinoAutomator : IDisposable
    {
        private SerialPort sp;
        private Func<Task<Vector2>> getMousePos;
        private Task init;
        public ArduinoAutomator(string port, Func<Task<Vector2>> getMousePos)
        {
        ...
        public ArduinoAutomator(Func<Task<Vector2>> getMousePos) : this(SerialPort.GetPortNames()[0], getMousePos) { }
        ...
        public float Scaling = 150.0f / 100;
        public async Task<Vector2> MousePos() => await getMousePos();
        ...
        Vector2 adjustorFactor;
        public async Task MouseMove(Vector2 d)
        {
            d *= 1 / Scaling;
        ...
        public async Task MouseTo(Vector2 p)
        {
            var d = p - await MousePos();

            await MouseMove(d);
        }
        public async Task WindMouseMove(Vector2 d)
        {
            d *= 1 / Scaling;
        ...
        public async Task WindMouseTo(Vector2 p)
        {
            var d = p - await MousePos();
        ...
        public class Test
        {
            ...
            public async Task TestMove()
            {
                ...
                var pp = new Vector2(100, -500);// Util.ReadPoint();
